          ...
          {dataProduct.length > 0 ? (
            <div className="absolute bg-white top-12 z-20 rounded-b-xl max-h-[250px] overflow-auto">
              <ul className="grid gap-2 p-4 md:w-[200px] lg:w-[650px] space-y-2">
                {dataProduct.map((sub) => {
                  const encryptData = encryptCrypto({
                    val: sub.id.toString(),
                    key: key,
                  });
                  return (
              ...
              <NavigationMenuItem key={item.menuName}>
                {item.submenu ? (
                  ...
                  <NavigationMenuLink
                    ...
                    asChild
                  >
                    <Link href={item.href as string}>{item.menuName}</Link>
                  </NavigationMenuLink>
                )}
              </NavigationMenuItem>
            );
          ...
          {!tokenExist ? (
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent
                side="bottom"
                align="end"
                className="min-w-44 p-2 rounded-xl bg-white shadow-lg"
              >
                {[
                  { kode: 1, title: "Dashboard", icon: <FiHome /> },
                  { kode: 2, title: "Logout", icon: <FiLogOut /> },
                ].map((item) => {
                  const baseClass =
                    "flex items-center gap-3 px-3 py-2 rounded-md select-none";
                  if (item.kode === 2) {
                    return (
                      <DropdownMenuItem
                        key={item.kode}
                  ...
                  return (
                    ...
                    >
                      <DropdownMenuItem
                        className={`${baseClass} text-gray-700 hover:bg-gray-100 cursor-pointer`}
                      >
                        {item.icon}
                        <span>{item.title}</span>
                      </DropdownMenuItem>
